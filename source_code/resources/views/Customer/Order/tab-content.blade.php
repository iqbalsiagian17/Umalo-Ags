            ...
            <div class="card-body">
                <div class="d-flex justify-content-between align-items-center mb-3">
                    <div>
                        ...
                        <p class="mb-1"><strong>{{ __('messages.order_date') }}:</strong>
                            {{ $order->created_at->format('d M Y') }}</p>
                        <p class="mb-1"><strong>{{ __('messages.status') }}:</strong> {{ $order->status }}</p>
                    </div>
                    <div>
                        <a href="{{ route('order.detail', $order->id) }}" class="btn btn-sm text-white"
                            style="background-color: #416bbf;">{{ __('messages.view_detail') }}</a>
                        @if (in_array($order->status, ['Diterima', 'Selesai']))
                            <a href="{{ route('order.generate_pdf', $order->id) }}"
                                class="btn btn-success btn-sm">{{ __('messages.download_invoice') }}</a>
                        @endif
                    </div>
                </div>
                    ...
                    <div class="d-flex mb-3">
                        <div style="flex: 1;">
                            @if ($item->produk->images->isNotEmpty())
                                <img src="{{ asset($item->produk->images->first()->gambar) }}"
                                    alt="{{ $item->produk->nama }}" style="width: 250px;" class="rounded">
                            @else
                ...
                <div class="d-flex justify-content-between">
                    <div>
                        @if ($order->status == 'Pengiriman' && $order->nomor_resi)
                            <p class="mt-2"><strong>{{ __('messages.tracking_number') }}:</strong>
                                {{ $order->nomor_resi }}</p>
                        @endif
                    </div>
                    <div>
                        <p class="text-danger"><strong>{{ __('messages.total_order') }}:
                                Rp{{ number_format($order->harga_total, 0, ',', '.') }}</strong></p>
                    </div>
                </div>
            </div>
        </div>
